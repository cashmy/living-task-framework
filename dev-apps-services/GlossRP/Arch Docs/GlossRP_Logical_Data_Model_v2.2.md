...
## Version 2.2 — Engine-Agnostic Schema for ECL-BASE Semantics
...
6. **Hybrid Category Model (Raw + Canonical)**  
   - Raw category labels from TermSpecs are always preserved on `TermVersion` (`raw_categories`).  
   - Canonical categories (`TermCategory` + `TermCategoryAssignment`) are added incrementally via ingestion and curation, not required for successful term ingestion.  
   - This supports emergent semantics at ECL-BASE while allowing curated, ECL-CORE-aligned categorization over time.
...
- `raw_categories`  
  - Type: `array<string>` or `JSONB`  
  - Semantics: raw category/domain labels as declared in the TermSpec (e.g., `["Framework", "Protocol", "UCS-Core"]`).  
  - Notes:  
    - Preserves the author’s original categorization intent.  
    - May contain labels that do not yet correspond to any canonical `TermCategory`.  
...
- Raw semantic intent is always captured without requiring prior canonical category definition.
...
Represents **canonical semantic groupings** for TermVersions.
...
- `name`  
  - Type: `string` (e.g., `varchar(128)`)  
  - Example: `Framework`, `Concept`, `Protocol`, `Subsystem`, `Tool`, `Role`.

- `description`  
  - Type: `text | null`

- `status`  
  - Type: enum (`active`, `pending`, `deprecated`)  
  - Semantics:  
    - `active` — canonical category available for assignment.  
    - `pending` — emerging category discovered from raw labels, awaiting human review.  
    - `deprecated` — no longer recommended; kept for historical purposes.
...
- `TermCategory` represents **curated canonical categories**, not every raw label from TermSpecs.  
- New categories may be created automatically with `status = 'pending'` or added manually via curation tools.

---
### 2.5 TermCategoryAssignment

Associates TermVersions with canonical TermCategories (**many-to-many**).
...
- Represents a **curated mapping** between a specific TermVersion and a canonical TermCategory.  
- A TermVersion may have zero, one, or many canonical category assignments.  
- Category-based retrieval for stable, “official” views (e.g., “all Framework terms”) should use `TermCategoryAssignment`.  
- Exploratory or curation-oriented views may also read from `TermVersion.raw_categories`.
