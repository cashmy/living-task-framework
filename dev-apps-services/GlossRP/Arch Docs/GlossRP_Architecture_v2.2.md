...
## Version 2.2 — ECL‑BASE Semantic Retrieval Microservice
...
- `raw_categories` (JSONB or text[]; raw category labels from TermSpecs, uncurated)
- `created_at` (timestamp)
- `updated_at` (timestamp)
...
Represents **canonical** categories. Does not store raw category labels extracted from TermSpecs.
...
Curated many‑to‑many mapping between TermVersion and canonical TermCategory. Raw categories do not enter this table.
...
4. **Storage Adapter Calls**
   ...
   - Does **not** create TermCategory or TermCategoryAssignment (these are handled exclusively by the Category Curation Tool).
...
### 8.1.6 Category Curation Endpoints (New in v2.2)

These endpoints expose **read-only or curation-pass-through** operations.  
GlossRP does not perform category logic internally; it passes requests to the external Category Curation Tool.

#### `GET /categories/raw/emerging`
Returns discovered raw category labels not yet mapped to canonical categories.

#### `GET /categories/raw/{label}`
Returns TermVersions associated with a raw label.

#### `POST /categories/promote`
Promotes a raw label into a canonical TermCategory (performed by the curation tool).

#### `POST /categories/merge`
Merges multiple raw labels into a single canonical category (performed by the curation tool).
