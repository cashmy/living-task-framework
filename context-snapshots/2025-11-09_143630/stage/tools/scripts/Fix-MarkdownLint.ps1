...
function Repair-MD004UnorderedListStyle {
  ...
  foreach ($line in $Lines) {
    # Special case: list item starting with bold text (e.g., "- **Text**")
    # Must use asterisk marker to avoid dash+asterisk conflict
    if ($line -match '^(\s*)-\s+(\*\*.+)$') {
      $indent = $matches[1]
      $content = $matches[2]
      $result += "$indent* $content"
    }
    # Normal case: Replace asterisk (*) or plus (+) with dash (-) for unordered lists
    # Only match if it's at start of line (with optional indent) AND followed by a space
    # This avoids matching bold text like **Author**
    elseif ($line -match '^(\s*)([*+])\s+(.*)$') {
      $indent = $matches[1]
      $content = $matches[3]
      # Check if content starts with ** (bold) - if so, keep asterisk marker
      if ($content -match '^\*\*.+') {
        $result += "$indent* $content"
      } else {
        $result += "$indent- $content"
...
function Repair-MalformedEmphasis {
  param([string[]]$Lines)
  $result = @()
  
  foreach ($line in $Lines) {
    # Fix pattern like "- *text**" which should be "- **text**"
    # This is malformed where single asterisk on left, double on right
    $fixed = $line -replace '^(\s*-\s+)\*([^*]+)\*\*\s*$', '$1**$2**'
...
foreach ($file in $files) {
  ...
  Write-Host "  [10/10] Fixing malformed emphasis markers..." -ForegroundColor Gray
  $lines = Repair-MalformedEmphasis -Lines $lines
  
  Write-Host "  [Final] Cleanup..." -ForegroundColor Gray
